...
Game::Game(UtilityEvaluator & evaluator, size_t width, size_t height, size_t figureSize, size_t colorsCount)
    : aField(width, height, evaluator), aUtilityEvaluator(evaluator), aScore(0)
{
}
...
size_t Game::play(UtilityEvaluator* evaluator)
{
    ...
    aScore = accumulate(game_scores.begin(), game_scores.end(), 0ULL, [](size_t a, size_t b) { return a + b; }) / game_scores.size();
