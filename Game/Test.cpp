...
size_t test_game::getUtility(const vector<double>& multipliers, int hz)
{
    ...
    switch (hz)
    {
        ...
        case 13:
            max = 7;
            min = 0;
            sum = accumulate(multipliers.begin(), multipliers.end(), (double) 0,
                                 [](const double& a, const double& b)
                                 {   return fabs(a) + fabs(b);});
            break;
        case 66:
            // formula: 0.34x1 + 0.55x2 -0.49x3 + 0.11x4 - 0.65x5 + 0.28x6 + 0.86x7
            sum = 100 - (fabs(0.34 - multipliers[0])
                + fabs(0.55 - multipliers[1])
                + fabs(-0.49 - multipliers[2])
                + fabs(0.11 - multipliers[3])
                + fabs(-0.65 - multipliers[4])
                + fabs(0.28 - multipliers[5])
